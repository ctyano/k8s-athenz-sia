...
func (idCfg *IdentityConfig) loadFromENV() error {
	...
	loadEnv("CERT_EXTRA_SAN_DNS", &idCfg.ServiceCert.RawCertExtraSANDNSs)
	...
	loadEnv("ROLE_CERT_DNS_SUFFIX", &idCfg.RoleCertDNSSuffix)
	...
	if len(idCfg.ServiceCert.RawCertExtraSANDNSs) > 0 {
		idCfg.ServiceCert.CertExtraSANDNSs = strings.Split(idCfg.ServiceCert.RawCertExtraSANDNSs, ",")
	}
...
func (idCfg *IdentityConfig) loadFromFlag(program string, args []string) error {
	...
	f.StringVar(&idCfg.DNSSuffix, "dns-suffix", idCfg.DNSSuffix, "DNS Suffix for x509 identity certificates (required for identity certificate provisioning)")
	...
	f.StringVar(&idCfg.RoleCertDNSSuffix, "role-cert-dns-suffix", idCfg.RoleCertDNSSuffix, "DNS Suffix for x509 role certificates (required for role certificate provisioning)")
	// RoleAuthHeader
